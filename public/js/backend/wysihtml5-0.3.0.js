...
window["rangy"] = (function () {
    ...
    function isTextRange(range) {
        return (
            range &&
            areHostMethods(range, textRangeMethods) &&
    ...
    function init() {
        ...
        if (isHostMethod(document, "createRange")) {
            ...
            if (
                areHostMethods(testRange, domRangeMethods) &&
        ...
        var body = isHostObject(document, "body")
            ? document.body
    ...
    function createMissingNativeApi(win) {
        ...
        // Notify listeners
        for (
            var i = 0, len = createMissingNativeApiListeners.length;
            i < len;
            ++i
        ) {
    ...
    api.requireModules = function (modules) {
        for (
            var i = 0, len = modules.length, module, moduleName;
            i < len;
            ++i
        ) {
...
rangy.createModule("DomUtil", function (api, module) {
    ...
    function getBody(doc) {
        return util.isHostObject(doc, "body")
            ? doc.body
    ...
    function comparePoints(nodeA, offsetA, nodeB, offsetB) {
        ...
        if (nodeA == nodeB) {
            ...
            childA =
                nodeA === root ? root : getClosestAncestorIn(nodeA, root, true);
            childB =
                nodeB === root ? root : getClosestAncestorIn(nodeB, root, true);
...
rangy.createModule("DomRange", function (api, module) {
    ...
    function isNonTextPartiallySelected(node, range) {
        return (
            node.nodeType != 3 &&
            (dom.isAncestorOf(node, range.startContainer, true) ||
                dom.isAncestorOf(node, range.endContainer, true))
        );
    }
    ...
    function inspect(range) {
        var name =
            typeof range.getName == "undefined" ? "Range" : range.getName();
        return (
            "[" +
            name +
            "(" +
        ...
        if (!range.collapsed) {
            ...
            if (this.sc === this.ec && dom.isCharacterDataNode(this.sc)) {
                ...
                this._first = this._next =
                    this.sc === root && !dom.isCharacterDataNode(this.sc)
                        ? this.sc.childNodes[this.so]
                        : dom.getClosestAncestorIn(this.sc, root, true);
                this._last =
                    this.ec === root && !dom.isCharacterDataNode(this.ec)
                        ? this.ec.childNodes[this.eo - 1]
                        : dom.getClosestAncestorIn(this.ec, root, true);
            }
        }
    }
    ...
    RangeIterator.prototype = {
        ...
        next: function () {
            ...
            if (current) {
                this._next =
                    current !== this._last ? current.nextSibling : null;

                // Check for partially selected text nodes
                if (
                    dom.isCharacterDataNode(current) &&
                    this.clonePartiallySelectedTextNodes
                ) {
                    if (current === this.ec) {
                        (current = current.cloneNode(true)).deleteData(
                            this.eo,
                            current.length - this.eo
                        );
                    }
    ...
    function assertSameDocumentOrFragment(node1, node2) {
        if (
            getDocumentOrFragmentContainer(node1, true) !==
            getDocumentOrFragmentContainer(node2, true)
        ) {
    ...
    function isOrphan(node) {
        return (
            !dom.arrayContains(rootContainerNodeTypes, node.nodeType) &&
        ...
        ? // Implementation as per HTML parsing spec, trusting in the browser's implementation of innerHTML. See
          ...
          function (fragmentStr) {
              ...
              if (
                  ...
                  (el.nodeName == "HTML" &&
                      dom.isHtmlNamespace(
                          dom.getDocument(el).documentElement
                      ) &&
    ...
    RangePrototype.prototype = {
        ...
        insertNode: function (node) {
            ...
            var firstNodeInserted = insertNodeAtPosition(
                node,
                this.startContainer,
                this.startOffset
            );
        ...
        cloneContents: function () {
            ...
            if (this.collapsed) {
                ...
                if (
                    ...
                    dom.isCharacterDataNode(this.startContainer)
                ) {
        ...
        canSurroundContents: function () {
            ...
            // no non-text nodes.
            var iterator = new RangeIterator(this, true);
            var boundariesInvalid =
                (iterator._first &&
                    isNonTextPartiallySelected(iterator._first, this)) ||
                (iterator._last &&
                    isNonTextPartiallySelected(iterator._last, this));
            iterator.detach();
        ...
        toString: function () {
            ...
            if (sc === this.endContainer && dom.isCharacterDataNode(sc)) {
                return sc.nodeType == 3 || sc.nodeType == 4
                    ? sc.data.slice(this.startOffset, this.endOffset)
                    : "";
            } else {
                var textBits = [],
        ...
        intersectsNode: function (node, touchingIsIntersecting) {
            ...
            var startComparison = dom.comparePoints(
                ...
                endComparison = dom.comparePoints(
                    parent,
                    offset + 1,
                    this.startContainer,
                    this.startOffset
        ...
        containsNodeContents: function (node) {
            return (
                this.comparePoint(node, 0) >= 0 &&
                this.comparePoint(node, dom.getNodeLength(node)) <= 0
            );
        },
    ...
    function createPrototypeRange(constructor, boundaryUpdater, detacher) {
        function createBeforeAfterNodeSetter(isBefore, isStart) {
            return function (node) {
                ...
                var boundary = (isBefore
                    ? getBoundaryBeforeNode
                    : getBoundaryAfterNode)(node);
                (isStart
                    ? setRangeStart
                    : setRangeEnd)(this, boundary.node, boundary.offset);
            };
        }
        ...
        function setRangeStart(range, node, offset) {
            ...
            if (node !== range.startContainer || offset !== range.startOffset) {
                ...
                if (
                    ...
                    dom.comparePoints(node, offset, ec, eo) == 1
                ) {
                    ec = node;
                    eo = offset;
                }
        ...
        function setRangeEnd(range, node, offset) {
            ...
            if (node !== range.endContainer || offset !== range.endOffset) {
                ...
                if (
                    ...
                    dom.comparePoints(node, offset, sc, so) == -1
                ) {
                    sc = node;
                    so = offset;
                }
                ...
                if (normalizeStart) {
                    if (dom.isCharacterDataNode(sc)) {
                        ...
                        if (so < sc.childNodes.length) {
                            ...
                            if (
                                startNode &&
                                dom.isCharacterDataNode(startNode)
                            ) {
    ...
    function updateCollapsedAndCommonAncestor(range) {
        ...
        range.commonAncestorContainer = range.collapsed
            ? range.startContainer
            : dom.getCommonAncestor(range.startContainer, range.endContainer);
    }
...
rangy.createModule("WrappedRange", function (api, module) {
    ...
    function getTextRangeContainerElement(textRange) {
        ...
        var startEndContainer =
            startEl == endEl ? startEl : dom.getCommonAncestor(startEl, endEl);

        return startEndContainer == parentEl
            ? startEndContainer
            : dom.getCommonAncestor(parentEl, startEndContainer);
    }
        ...
        if (
            ...
            if (/[\r\n]/.test(boundaryNode.data)) {
                ...
                */
                ...
                while (
                    (comparison = tempRange.compareEndPoints(
                        "StartToEnd",
                        tempRange
                    )) == -1
                ) {
            ...
            previousNode =
                (isCollapsed || !isStart) && workingNode.previousSibling;
    ...
    function createBoundaryTextRange(boundaryPosition, isStart) {
        ...
        if (nodeIsDataNode) {
            ...
            boundaryNode =
                boundaryOffset < childNodes.length
                    ? childNodes[boundaryOffset]
                    : null;
...
*/
            ...
            if (
                range.startContainer == testTextNode &&
            ...
            if (api.util.isHostMethod(range, "createContextualFragment")) {
                rangeProto.createContextualFragment = function (fragmentStr) {
                    return this.nativeRange.createContextualFragment(
                        fragmentStr
                    );
                };
            }
        ...
        WrappedRange.prototype.refresh = function () {
            ...
            var rangeContainerElement = getTextRangeContainerElement(
                this.textRange
            );
...
rangy.createModule("WrappedSelection", function (api, module) {
    ...
    if (useDocumentSelection) {
        ...
        api.isSelectionValid = function (winParam) {
            ...
            return (
                nativeSel.type != "None" ||
                dom.getDocument(nativeSel.createRange().parentElement()) == doc
            );
        };
    ...
    if (selectionHasAnchorAndFocus) {
        ...
        selectionIsCollapsed = function (sel) {
            return sel.rangeCount
                ? sel.getRangeAt(sel.rangeCount - 1).collapsed
                : false;
        };
    }
    ...
    function getNativeRange(range) {
        ...
        if (range instanceof DomRange) {
            ...
            if (!nativeRange) {
                nativeRange = api.createNativeRange(
            ...
            api.features.implementsDomRange &&
            range instanceof dom.getWindow(range.startContainer).Range
        ) {
            nativeRange = range;
        }
        return nativeRange;
    ...
    function updateControlSelection(sel) {
        ...
        if (sel.docSelection.type == "None") {
            ...
            if (isTextRange(controlRange)) {
                ...
                sel.isCollapsed =
                    sel.rangeCount == 1 && sel._ranges[0].collapsed;
    ...
    if (
        ...
        util.areHostMethods(testSelection, ["removeAllRanges", "addRange"])
    ) {
                    ...
                    if (backwards && selectionHasExtend) {
                        ...
                        if (this.rangeCount == previousRangeCount + 1) {
                            ...
                            if (api.config.checkSelectionRanges) {
                                ...
                                if (
                                    nativeRange &&
                                    !DomRange.rangesEqual(nativeRange, range)
                                ) {
        ...
        util.isHostMethod(testSelection, "empty") &&
                ...
                if (this.docSelection.type != "None") {
                    ...
                    if (this.anchorNode) {
                        ...
                        if (controlRange.length) {
                            doc = dom
                                .getDocument(controlRange.item(0))
                                .body.createTextRange();
                        }
                    }
                    if (doc) {
        ...
        refreshSelection = function (sel) {
            if (
                ...
                sel._ranges.length = sel.rangeCount =
                    sel.nativeSelection.rangeCount;
                if (sel.rangeCount) {
                    for (var i = 0, len = sel.rangeCount; i < len; ++i) {
                        sel._ranges[i] = new api.WrappedRange(
                            sel.nativeSelection.getRangeAt(i)
                        );
                    }
                    updateAnchorAndFocusFromRange(
                        sel,
    ...
    function assertNodeInSameDocument(sel, node) {
        if (
            sel.anchorNode &&
            dom.getDocument(sel.anchorNode) !== dom.getDocument(node)
        ) {
            throw new DOMException("WRONG_DOCUMENT_ERR");
        }
    }
    ...
    function inspect(sel) {
        ...
        var name =
            typeof sel.getName == "function" ? sel.getName() : "Selection";
    ...
    var klass = (proto.constructor = function () {
        if (!Base._prototyping) {
            if (this._constructing || this.constructor == klass) {
                ...
                // casting
                return (arguments[0].extend || extend).call(
                    arguments[0],
                    proto
                );
            }
...
wysihtml5.browser = (function () {
    ...
    function iosVersion(userAgent) {
        return ((/ipad|iphone|ipod/.test(userAgent) &&
            userAgent.match(/ os (\d+).+? like mac os x/)) || [, 0])[1];
    }
    ...
    return {
        ...
        supportsNativeGetElementsByClassName: function () {
            return (
                String(document.getElementsByClassName).indexOf(
                    "[native code]"
                ) !== -1
            );
        },
    ...
    /** @scope wysihtml5.lang.Dialog.prototype */ {
        ...
        on: function () {
            return this.observe.apply(
                this,
                wysihtml5.lang.array(arguments).get()
            );
        },
...
(function () {
    ...
    wysihtml5.lang.string = function (str) {
        ...
        return {
            ...
            trim: function () {
                return str
                    .replace(WHITE_SPACE_START, "")
                    .replace(WHITE_SPACE_END, "");
            },
...
(function (wysihtml5) {
    var /**
         ...
         */
        IGNORE_URLS_IN = wysihtml5.lang.array([
            "CODE",
            "PRE",
            "A",
            "SCRIPT",
    ...
    function _getTempElement(context) {
        ...
        if (!tempElement) {
            tempElement = context._wysihtml5_tempElement = context.createElement(
                "div"
            );
        }
        return tempElement;
    ...
    function _wrapMatchesInNode(textNode) {
        ...
        tempElement.innerHTML =
            "<span></span>" + _convertUrlsToLinks(textNode.data);
    ...
    function _parseNode(element) {
        ...
        if (
            element.nodeType === wysihtml5.TEXT_NODE &&
    ...
    function convertToList(element, listType) {
        ...
        for (i = 0; i < lineBreaksLength; i++) {
            ...
            while (
                ...
                if (
                    wysihtml5.dom.getStyle("display").from(parentNode) ===
                    "block"
                ) {
        ...
        for (i = 0; i < childNodesLength; i++) {
            ...
            isBlockElement =
                wysihtml5.dom.getStyle("display").from(childNode) === "block";
            ...
            return {
                to: function (elementToCopyTo) {
                    ...
                    for (; i < length; i++) {
                        ...
                        if (
                            typeof elementToCopyFrom[attribute] !==
                                "undefined" &&
                            elementToCopyFrom[attribute] !== ""
                        ) {
                            elementToCopyTo[attribute] =
                                elementToCopyFrom[attribute];
                        }
                    }
...
(function (dom) {
    ...
    var shouldIgnoreBoxSizingBorderBox = function (element) {
        if (hasBoxSizingBorderBox(element)) {
            return (
                parseInt(dom.getStyle("width").from(element), 10) <
                element.offsetWidth
            );
        }
        return false;
    ...
    var hasBoxSizingBorderBox = function (element) {
        ...
        for (; i < length; i++) {
            if (
                dom.getStyle(BOX_SIZING_PROPERTIES[i]).from(element) ===
                "border-box"
            ) {
 ...
 */
(function (wysihtml5) {
    wysihtml5.dom.delegate = function (
        container,
        selector,
        eventName,
        handler
...
wysihtml5.dom.getParentElement = (function () {
    ...
    function _getParentElementWithNodeNameAndClassName(
        node,
        nodeName,
        className,
        classRegExp,
    ...
    return function (node, matchingSet, levels) {
        ...
        if (matchingSet.className || matchingSet.classRegExp) {
            return _getParentElementWithNodeNameAndClassName(
                node,
                matchingSet.nodeName,
                matchingSet.className,
                matchingSet.classRegExp,
                levels
            );
        } else {
...
(function (wysihtml5) {
    ...
    wysihtml5.dom.hasElementWithClassName = function (doc, className) {
        ...
        var key = _getDocumentIdentifier(doc) + ":" + className,
            cacheEntry = LIVE_CACHE[key];
        if (!cacheEntry) {
            cacheEntry = LIVE_CACHE[key] = doc.getElementsByClassName(
                className
            );
        }
...
wysihtml5.dom.parse = (function () {
    ...
    function parse(elementOrHtml, rules, context, cleanUp) {
        ...
        return isString
            ? wysihtml5.quirks.getCorrectInnerHTML(element)
            : element;
    }
    ...
    function _handleAttributes(oldNode, newNode, rule) {
        ...
        if (checkAttributes) {
            for (attributeName in checkAttributes) {
                ...
                newAttributeValue = method(
                    _getAttribute(oldNode, attributeName)
                );
    ...
    function _getAttribute(node, attributeName) {
        ...
        if (
            ...
            var outerHTML = node.outerHTML.toLowerCase(),
                ...
                hasAttribute =
                    outerHTML.indexOf(" " + attributeName + "=") != -1;
...
wysihtml5.dom.removeEmptyTextNodes = function (node) {
    ...
    for (; i < childNodesLength; i++) {
        childNode = childNodes[i];
        if (
            childNode.nodeType === wysihtml5.TEXT_NODE &&
            childNode.data === ""
        ) {
...
wysihtml5.dom.renameElement = function (element, newNodeName) {
    ...
    wysihtml5.dom
        .copyAttributes(["align", "className"])
        .from(element)
        .to(newElement);
...
(function (dom) {
    ...
    function resolveList(list) {
        ...
        var doc = list.ownerDocument,
            ...
            previousSibling =
                list.previousElementSibling || list.previousSibling,
            firstChild,
            lastChild,
            isLastChild,
    ...
    wysihtml5.dom.Sandbox = Base.extend(
        /** @scope wysihtml5.dom.Sandbox.prototype */ {
            ...
            _createIframe: function () {
                ...
                if (
                    wysihtml5.browser.throwsMixedContentWarningWhenIframeSrcIsEmpty()
                ) {
            ...
            _onLoadIframe: function (iframe) {
                ...
                iframeWindow.onerror = function (
                    ...
                    throw new Error(
                        "wysihtml5.Sandbox: " + errorMessage,
                        fileName,
                        lineNumber
                    );
                };
                ...
                if (!wysihtml5.browser.supportsSandboxedIframes()) {
                    ...
                    var i, length;
                    for (
                        i = 0, length = windowProperties.length;
                        i < length;
                        i++
                    ) {
                    ...
                    for (
                        i = 0, length = windowProperties2.length;
                        i < length;
                        i++
                    ) {
                        this._unset(
                    ...
                    for (
                        i = 0, length = documentProperties.length;
                        i < length;
                        i++
                    ) {
    ...
    wysihtml5.quirks.ensureProperClearingOfLists = (function () {
        ...
        var clearIfNecessary = function (element, contentEditableElement) {
            if (
                ...
                !wysihtml5.lang
                    .array(ELEMENTS_THAT_CONTAIN_LI)
                    .contains(contentEditableElement.firstChild.nodeName)
            ) {
                return;
            }
            ...
            var listIsFirstChildOfContentEditable =
                list == contentEditableElement.firstChild;
            ...
            var onlyListItemIsEmpty = list.firstChild
...
(function (wysihtml5) {
    ...
    wysihtml5.quirks.getCorrectInnerHTML = function (element) {
        ...
        var elementsWithTilde = element.querySelectorAll(
                "[href*='~'], [src*='~']"
            ),
            url,
            urlToSearch,
        ...
        for (i = 0, length = elementsWithTilde.length; i < length; i++) {
            ...
            urlToSearch = wysihtml5.lang
                .string(url)
                .replace("~")
                .by(TILDE_ESCAPED);
            innerHTML = wysihtml5.lang
                .string(innerHTML)
                .replace(urlToSearch)
                .by(url);
        }
...
(function (wysihtml5) {
    var dom = wysihtml5.dom,
        USE_NATIVE_LINE_BREAK_WHEN_CARET_INSIDE_TAGS = [
            "LI",
            "P",
            "H1",
            "H2",
    ...
    wysihtml5.quirks.insertLineBreakOnReturn = function (composer) {
        function unwrap(selectedNode) {
            var parentElement = dom.getParentElement(
                selectedNode,
                { nodeName: ["P", "DIV"] },
                2
            );
            ...
            var invisibleSpace = document.createTextNode(
        ...
        function keyDown(event) {
            ...
            var element = event.target,
                ...
                blockElement = dom.getParentElement(
                    selectedNode,
            ...
            if (blockElement) {
                ...
                if (
                    ...
                    blockElement.nodeName.match(/H[1-6]/) &&
                    keyCode === wysihtml5.ENTER_KEY
                ) {
                    setTimeout(function () {
        ...
        /** @scope wysihtml5.Selection.prototype */ {
            ...
            selectNode: function (node) {
                var range = rangy.createRange(this.doc),
                    ...
                    isEmpty =
                        content === "" || content === wysihtml5.INVISIBLE_SPACE,
            ...
            executeAndRestoreSimple: function (method) {
                ...
                lastNode =
                    textNodes[textNodes.length - 1] || range.endContainer;
                ...
                try {
                    newRange.setStart(
                        rangeBackup.startContainer,
                        rangeBackup.startOffset
                    );
                } catch (e1) {}
                try {
                    newRange.setEnd(
                        rangeBackup.endContainer,
            ...
            scrollIntoView: function () {
                var doc = this.doc,
                    ...
                    tempElement = (doc._wysihtml5ScrollIntoViewElement =
                        doc._wysihtml5ScrollIntoViewElement ||
                        (function () {
...
(function (wysihtml5, rangy) {
    ...
    function hasSameClasses(el1, el2) {
        return (
            el1.className.replace(REG_EXP_WHITE_SPACE, " ") ==
            el2.className.replace(REG_EXP_WHITE_SPACE, " ")
        );
    }
    ...
    function elementsHaveSameNonClassAttributes(el1, el2) {
        ...
        for (
            var i = 0, len = el1.attributes.length, attr1, attr2, name;
            i < len;
            ++i
        ) {
    ...
    function splitNodeAt(node, descendantNode, descendantOffset) {
        ...
        if (rangy.dom.isCharacterDataNode(descendantNode)) {
            if (descendantOffset == 0) {
                ...
                newNode = rangy.dom.splitDataNode(
                    descendantNode,
                    descendantOffset
                );
            }
        ...
        return descendantNode == node
            ? newNode
            : splitNodeAt(
                  node,
                  newNode.parentNode,
                  rangy.dom.getNodeIndex(newNode)
              );
    }
    ...
    HTMLApplier.prototype = {
        getAncestorWithClass: function (node) {
            var cssClassMatch;
            while (node) {
                cssClassMatch = this.cssClass
                    ? hasClass(node, this.cssClass, this.similarClassRegExp)
                    : true;
                if (
                    node.nodeType == wysihtml5.ELEMENT_NODE &&
                    rangy.dom.arrayContains(
                        this.tagNames,
        ...
        isAppliedToRange: function (range) {
            ...
            for (
                var i = 0, len = textNodes.length, selectedText;
                i < len;
                ++i
            ) {
...
(function (wysihtml5) {
    ...
    function _format(composer, attributes) {
        ...
        if (length === 1) {
            ...
            isEmpty =
                textContent === "" || textContent === wysihtml5.INVISIBLE_SPACE;
...
(function (wysihtml5) {
    ...
    wysihtml5.commands.formatBlock = {
        exec: function (composer, command, nodeName, className, classRegExp) {
            ...
            if (
                nodeName === null ||
                wysihtml5.lang.array(BLOCK_ELEMENTS_GROUP).contains(nodeName)
            ) {
                selectedNode = composer.selection.getSelectedNode();
                ...
                if (blockElement) {
                    composer.selection.executeAndRestoreSimple(function () {
                        ...
                        if (nodeName) {
                            blockElement = dom.renameElement(
                                blockElement,
                                nodeName
                            );
                        }
...
(function (wysihtml5) {
    ...
    function _getTagNames(tagName) {
        ...
        return alias
            ? [tagName.toLowerCase(), alias.toLowerCase()]
            : [tagName.toLowerCase()];
    }
    ...
    wysihtml5.commands.formatInline = {
        ...
        state: function (composer, command, tagName, className, classRegExp) {
            ...
            if (
                !wysihtml5.dom.hasElementWithTagName(doc, tagName) &&
...
(function (wysihtml5) {
    var undef,
        LINE_BREAK =
            "<br>" + (wysihtml5.browser.needsSpaceAfterLineBreak() ? " " : "");
...
(function (wysihtml5) {
    var Z_KEY = 90,
        ...
        UNDO_HTML =
            '<span id="_wysihtml5-undo" class="_wysihtml5-temp">' +
            wysihtml5.INVISIBLE_SPACE +
            "</span>",
        REDO_HTML =
            '<span id="_wysihtml5-redo" class="_wysihtml5-temp">' +
            wysihtml5.INVISIBLE_SPACE +
            "</span>",
        dom = wysihtml5.dom;
            ...
            _observe: function () {
                ...
                if (wysihtml5.browser.hasUndoInContextMenu()) {
                    ...
                    dom.observe(this.element, "contextmenu", function () {
                        cleanUp();
                        that.composer.selection.executeAndRestoreSimple(
                            function () {
                                if (that.element.lastChild) {
                                    that.composer.selection.setAfter(
...
wysihtml5.views.View = Base.extend(
    /** @scope wysihtml5.views.View.prototype */ {
        ...
        focus: function () {
            if (
                this.element.ownerDocument.querySelector(":focus") ===
                this.element
            ) {
                return;
            }
    ...
    wysihtml5.views.Composer = wysihtml5.views.View.extend(
        /** @scope wysihtml5.views.Composer.prototype */ {
            ...
            getValue: function (parse) {
                var value = this.isEmpty()
                    ? ""
                    : wysihtml5.quirks.getCorrectInnerHTML(this.element);
                ...
                value = wysihtml5.lang
                    .string(value)
                    .replace(wysihtml5.INVISIBLE_SPACE)
                    .by("");
            ...
            focus: function (setToEnd) {
                ...
                if (
                    wysihtml5.browser.doesAsyncFocus() &&
            ...
            isEmpty: function () {
                ...
                return (
                    ...
                    this.hasPlaceholderSet() ||
                    (this.getTextContent() === "" &&
                        !this.element.querySelector(elementsWithVisualValue))
                );
            },
            ...
            _initAutoLinking: function () {
                ...
                var // Use a live NodeList to check whether there are any links in the document
                    ...
                    getTextContent = function (element) {
                        var textContent = wysihtml5.lang
                            .string(dom.getTextContent(element))
                            .trim();
...
(function (wysihtml5) {
    ...
    wysihtml5.views.Composer.prototype.style = function () {
        var that = this,
            ...
            originalPlaceholder =
                hasPlaceholder && textareaElement.getAttribute("placeholder");
        ...
        dom.copyStyles(BOX_FORMATTING)
            .from(textareaElement)
            .to(this.iframe)
            .andTo(this.blurStylesHost);

        // --------- editor styles ---------
        dom.copyStyles(TEXT_FORMATTING)
            .from(textareaElement)
            .to(this.element)
            .andTo(this.blurStylesHost);
        ...
        var boxFormattingStyles = wysihtml5.lang
            .array(BOX_FORMATTING)
            .without(["display"]);
        ...
        if (!wysihtml5.browser.hasCurrentStyleProperty()) {
            var winObserver = dom.observe(win, "resize", function () {
                ...
                var originalTextareaDisplayStyle = dom
                        .getStyle("display")
                        .from(textareaElement),
                    originalComposerDisplayStyle = dom
                        .getStyle("display")
                        .from(that.iframe);
        ...
        this.parent.observe("focus:composer", function () {
            dom.copyStyles(boxFormattingStyles)
                .from(that.focusStylesHost)
                .to(that.iframe);
            dom.copyStyles(TEXT_FORMATTING)
                .from(that.focusStylesHost)
                .to(that.element);
...
(function (wysihtml5) {
    ...
    wysihtml5.views.Composer.prototype.observe = function () {
        var that = this,
            ...
            element = this.element,
            focusBlurElement = browser.supportsEventsInIframeCorrectly()
                ? element
                : this.sandbox.getWindow(),
            // Firefox < 3.5 doesn't support the drop event, instead it supports a so called "dragdrop" event which behaves almost the same
            pasteEvents = browser.supportsEvent("drop")
                ? ["drop", "paste"]
                : ["dragdrop", "paste"];
        ...
        /** @scope wysihtml5.views.Synchronizer.prototype */ {
            ...
            fromComposerToTextarea: function (shouldParseHtml) {
                this.textarea.setValue(
                    wysihtml5.lang.string(this.composer.getValue()).trim(),
                    shouldParseHtml
                );
            },
...
wysihtml5.views.Textarea = wysihtml5.views.View.extend(
    /** @scope wysihtml5.views.Textarea.prototype */ {
        ...
        hasPlaceholderSet: function () {
            var supportsPlaceholder = wysihtml5.browser.supportsPlaceholderAttributeOn(
                    this.element
                ),
                placeholderText =
                    this.element.getAttribute("placeholder") || null,
        ...
        isEmpty: function () {
            return (
                !wysihtml5.lang.string(this.element.value).trim() ||
                this.hasPlaceholderSet()
            );
        },
    ...
    wysihtml5.toolbar.Dialog = wysihtml5.lang.Dispatcher.extend(
        /** @scope wysihtml5.toolbar.Dialog.prototype */ {
            ...
            _observe: function () {
                ...
                var formElements = this.container.querySelectorAll(
                        SELECTOR_FORM_ELEMENTS
                    ),
                    i = 0,
            ...
            _interpolate: function (avoidHiddenFields) {
                ...
                for (; i < length; i++) {
                    ...
                    newValue = this.elementToChange
                        ? this.elementToChange[fieldName] || ""
                        : field.defaultValue;
            ...
            show: function (elementToChange) {
                var that = this,
                    firstField = this.container.querySelector(
                        SELECTOR_FORM_ELEMENTS
                    );
    ...
    wysihtml5.toolbar.Toolbar = Base.extend(
        /** @scope wysihtml5.toolbar.Toolbar.prototype */ {
            constructor: function (editor, container) {
                ...
                var speechInputLinks = this.container.querySelectorAll(
            ...
            _getLinks: function (type) {
                var links = (this[type + "Links"] = wysihtml5.lang
                        .array(
                ...
                for (; i < length; i++) {
                    ...
                    value = link.getAttribute(
                        "data-wysihtml5-" + type + "-value"
                    );
                    group = this.container.querySelector(
                        "[data-wysihtml5-" + type + "-group='" + name + "']"
                    );
            ...
            _getDialog: function (link, command) {
                var that = this,
                    dialogElement = this.container.querySelector(
                        "[data-wysihtml5-dialog='" + command + "']"
                    ),
                    dialog,
                    caretBookmark;
            ...
            execCommand: function (command, commandValue) {
                ...
                var commandObj = this.commandMapping[
            ...
            _observe: function () {
                ...
                dom.delegate(
                    ...
                    function (event) {
                        var link = this,
                            command = link.getAttribute(
                                "data-wysihtml5-command"
                            ),
                            commandValue = link.getAttribute(
            ...
            _updateLinkStates: function () {
                ...
                for (i in commandMapping) {
                    ...
                    if (this.commandsDisabled) {
                        ...
                        state = this.composer.commands.state(
                            command.name,
                            command.value
                        );
        ...
        /** @scope wysihtml5.Editor.prototype */ {
            constructor: function (textareaElement, config) {
                this.textareaElement =
                    typeof textareaElement === "string"
                        ? document.getElementById(textareaElement)
                        : textareaElement;
                this.config = wysihtml5.lang
                    .object({})
                    .merge(defaultConfig)
                    .merge(config)
                    .get();
                this.textarea = new wysihtml5.views.Textarea(
                    this,
                    this.textareaElement,
                    this.config
                );
                ...
                if (
                    ...
                    (!this.config.supportTouchDevices &&
                        wysihtml5.browser.isTouchDevice())
                ) {
                    var that = this;
                    setTimeout(function () {
                ...
                wysihtml5.dom.addClass(
                    document.body,
                    this.config.bodyClassName
                );

                this.composer = new wysihtml5.views.Composer(
                    this,
                    this.textareaElement,
                    this.config
                );
                ...
                this.observe("beforeload", function () {
                    ...
                    if (this.config.toolbar) {
                        this.toolbar = new wysihtml5.toolbar.Toolbar(
                            this,
                            this.config.toolbar
                        );
                    }
            ...
            parse: function (htmlOrElement) {
                var returnValue = this.config.parser(
                    htmlOrElement,
