...
<div class="container-fluid">
    ...
    <div class="span8">
      <div class="widget-box">
        <div class="widget-content nopadding">
          <form  method="POST" action="/participated-sports/{{$user->id}}/update/"class="form-horizontal">
          @csrf
          @method('PUT')
  ...
  <option value="{{$user->id}}"@foreach($eventUsers as  $item){{$item->pivot->user_id  == $user->id ? 'selected': ''}}   @endforeach>{{$user->first_name}}</option>
  @endforeach
          ...
          <div class="control-group">
              ...
              {{ $trackEvent->event_name }} &nbsp; <input type="checkbox" name="events[]" value="{{ $trackEvent->id }}" @foreach($events as $item){{$item->id == $trackEvent->id ? 'checked': ''}}   @endforeach>
@endforeach 
              </div>
            </div>
            <div class="control-group">
              ...
              {{ $fieldEvent->event_name }} &nbsp; <input type="checkbox" name="events2[]" value="{{ $fieldEvent->id }}"  @foreach($fields as $item){{$item->id == $fieldEvent->id ? 'checked': ''}}   @endforeach>
@endforeach 
              </div>
            </div>
